...
매일 자동으로 가져와, 학교 홈페이지보다 보기 편한 웹페이지와 캘린더
구독(.ics) 피드로 제공합니다.
...
(KST 06:00) 최신 데이터를 조회해 `docs/kcu-schedule.ics`(캘린더 구독용)와
`docs/kcu-schedule.json`(웹페이지용)을 갱신하고, 변경이 있을 때만
커밋합니다. GitHub Pages가 `main` 브랜치의 `/docs` 폴더로 설정되어 있어
위 웹페이지와 구독 URL이 그대로 동작합니다.
